...
-- SAFE version - handles servers that don't have getILvlMacc() yet
xi.additionalEffect.getMagicHitRateForStatusAmmo = function(attacker, target)
    if not attacker or not target then
        return 0
    end
    local maccGear = attacker:getMod(xi.mod.MACC) or 0
    local ilvlMacc = 0

    local rangedItem = attacker:getEquippedItem(xi.slot.RANGED)
    if rangedItem then
        if rangedItem.getILvlMacc then
            ilvlMacc = rangedItem:getILvlMacc() or 0
        end
    else
        local ammoItem = attacker:getEquippedItem(xi.slot.AMMO)
        if ammoItem then
            if ammoItem.getILvlMacc then
                ilvlMacc = ammoItem:getILvlMacc() or 0
            end
        end
    end

    local pINT = attacker:getStat(xi.mod.INT) or 0
    local effectiveMACC = maccGear + ilvlMacc + math.floor(pINT / 2)

    local mevaMod = target:getMod(xi.mod.MEVA) or 0
    local tMND = target:getStat(xi.mod.MND) or 0
    local effectiveMEVA = mevaMod + math.floor(tMND / 2)

    local diff = effectiveMACC - effectiveMEVA
    if diff < 0 then
        diff = math.floor(diff / 2)
    end

    local hitRate = utils.clamp((50 + diff) / 100, 0.05, 0.95)
    return hitRate
end
...
xi.additionalEffect.calcPhysDamage = function(attacker, defender, item, params)
    ...
    if math.random(1, 100) <= defender:getMod(xi.mod.NULL_DAMAGE) then return 0 end
    if params.isPhysical and math.random(1, 100) <= defender:getMod(xi.mod.NULL_PHYSICAL_DAMAGE) then return 0 end
    if params.isRanged and math.random(1, 100) <= defender:getMod(xi.mod.NULL_RANGED_DAMAGE) then return 0 end
    if params.isBreath and math.random(1, 100) <= defender:getMod(xi.mod.NULL_BREATH_DAMAGE) then return 0 end

    if math.random(1, 100) <= defender:getMod(xi.mod.ABSORB_DMG_CHANCE) then return params.damage * -1 end
    if (params.isPhysical or params.isRanged) and math.random(1, 100) <= defender:getMod(xi.mod.PHYS_ABSORB) then return params.damage * -1 end
    ...
    {
        [xi.damageType.PIERCING] = function() params.damage = params.damage * (1 + defender:getMod(xi.mod.PIERCE_SDT) / 10000) end,
        [xi.damageType.SLASHING] = function() params.damage = params.damage * (1 + defender:getMod(xi.mod.SLASH_SDT) / 10000) end,
        [xi.damageType.BLUNT] = function() params.damage = params.damage * (1 + defender:getMod(xi.mod.IMPACT_SDT) / 10000) end,
        [xi.damageType.HTH] = function() params.damage = params.damage * (1 + defender:getMod(xi.mod.HTH_SDT) / 10000) end,
    }
...
-- DEBUFF handler for status ammunition (bolts, arrows, etc.)
-- DB rates are intentionally set to 100% on most servers.
-- Final inflict rate is therefore determined almost entirely by the magic hit rate
-- (capped at 95% unless the DB chance is intentionally lowered).
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.DEBUFF] = function(actor, target, item, params)
    local subEffect = params.subEffect
    local effectId = params.addStatus

    if xi.data.statusEffect.isEffectNullified(target, effectId, 0) then
        return 0, 0, 0
    end

    -- Stage 1: Base rate randomizer (ITEM_ADDEFFECT_CHANCE from DB)
	-- DB Rates currently set to 100, so this check is redundant. highly suggest clamping rates or modifiying DB.
    if math.random(100) > (params.chance or 0) then
        return 0, 0, 0
    end

    -- Stage 2: Magic accuracy hit rate (5-95%)
    local hitRate = params.magicHitRate or 1.0
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.HP_DRAIN] = function(attacker, defender, item, params)
    ...
    local damage = xi.additionalEffect.calcDamage(attacker, params.element, defender, params.damage)
    if defender:isUndead() then return 0, 0, 0 end
    if damage > defender:getHP() then damage = defender:getHP() end
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.MP_DRAIN] = function(attacker, defender, item, params)
    ...
    local damage = xi.additionalEffect.calcDamage(attacker, params.element, defender, params.damage)
    if defender:isUndead() then return 0, 0, 0 end
    if damage > defender:getMP() then damage = defender:getMP() end
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.TP_DRAIN] = function(attacker, defender, item, params)
    ...
    if defender:isUndead() then return 0, 0, 0 end
    local damage = xi.additionalEffect.calcDamage(attacker, params.element, defender, params.damage)
    if damage > defender:getTP() then damage = defender:getTP() end
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.SELF_BUFF] = function(attacker, defender, item, params)
    ...
    local msgID = 0
    local msgParam = 0
    if params.addStatus == xi.effect.BLINK then
        if attacker:hasStatusEffect(xi.effect.BLINK) or attacker:hasStatusEffect(xi.effect.COPY_IMAGE) then
            return 0, 0, 0
        else
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.DEATH] = function(attacker, defender, item, params)
    ...
    local msgID = 0
    local msgParam = 0
    if defender:isNM() or defender:isUndead() or math.random(1, 100) > defender:getMod(xi.mod.DEATHRES) then
        return 0, 0, 0
    else
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.HPMP_DRAIN] = function(attacker, defender, item, params)
    local drainRoll = math.random(1, 2)
    local drainFuncs = { [1] = xi.additionalEffect.procType.HP_DRAIN, [2] = xi.additionalEffect.procType.MP_DRAIN }
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.HPMPTP_DRAIN] = function(attacker, defender, item, params)
    local drainRoll = math.random(1, 3)
    local drainFuncs = { [1] = xi.additionalEffect.procType.HP_DRAIN, [2] = xi.additionalEffect.procType.MP_DRAIN, [3] = xi.additionalEffect.procType.TP_DRAIN }
    return xi.additionalEffect.procFunctions[drainFuncs[drainRoll]](attacker, defender, item, params)
end
...
xi.additionalEffect.nmSpecificConfigs = {
    ...
    ['Seiryu'] = {
        ...
        specialAction = function(defender) defender:setMobMod(xi.mobMod.ADD_EFFECT, 0) end,
    },
    ['Genbu'] = {
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.NM_SPECIFIC] = function(attacker, defender, item, params)
    ...
    if config and (config.requiredItem == item:getID() or config.requiredItem == xi.item.NONE) then
        local damage = xi.additionalEffect.calcDamage(attacker, params.element, defender, params.damage)
        msgID = xi.msg.basic.ADD_EFFECT_DMG
        msgParam = damage
        if config.specialAction then config.specialAction(defender) end
        subEffect = config.customSubEffect or subEffect
        msgID = config.customMsgID or msgID
        msgParam = config.customMsgParam or msgParam
    else
        if defender and item then defender:setLocalVar('aeFromItemId', item:getID()) end
        return 0, 0, 0
    end
    return subEffect, msgID, msgParam
end
...
xi.additionalEffect.attack = function(attacker, defender, baseAttackDamage, item)
    ...
    -- Status ammunition (bolts etc.) uses TWO-STAGE check
    if params.addType == xi.additionalEffect.procType.DEBUFF then
        params.magicHitRate = xi.additionalEffect.getMagicHitRateForStatusAmmo(attacker, defender)
    else
