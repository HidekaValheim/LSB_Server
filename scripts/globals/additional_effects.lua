...
-- NEW: Two-stage magic accuracy check for status ammunition (bolts, etc.)
-- Uses ONLY player MACC + ilvlMacc (RANGED priority + AMMO fallback) + INT/2 vs monster MEVA + MND/2
-- (exactly the same base math as magic_hit_rate.lua, no skill/element/etc.)
...
xi.additionalEffect.getMagicHitRateForStatusAmmo = function(attacker, target)
    ...
    -- Player Magic Accuracy (gear/food + ilvlMacc from RANGED or AMMO + INT contribution)
    local maccGear = attacker:getMod(xi.mod.MACC) or 0
    local ilvlMacc = 0
    -- Prioritize RANGED slot (bow/gun/crossbow/chakram/boomerang)
    ...
    if rangedItem then
        ...
        -- Fallback to AMMO slot (shurikens/winds/etc) when ranged slot is empty
    ...
    -- Target Magic Evasion (mods + MND contribution)
...
xi.additionalEffect.procFunctions[xi.additionalEffect.procType.DEBUFF] = function(actor, target, item, params)
    ...
    -- Stage 1: Base rate randomizer (ITEM_ADDEFFECT_CHANCE from DB) - done FIRST
    ...
    local duration = params.duration -- full duration, no resist scaling for ammo
...
xi.additionalEffect.attack = function(attacker, defender, baseAttackDamage, item)
    ...
    --   1. Base rate randomizer (ITEM_ADDEFFECT_CHANCE)
    --   2. Magic accuracy hit rate (5-95%)
    ...
    -- This is the damage of the auto attack that generated this function call
    params.baseAttackDamage = baseAttackDamage

    -- dStat bonus (INT/MND for ranged ammo, etc.)
